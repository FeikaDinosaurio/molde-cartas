...
func _ready():
	...
	hand_node = get_tree().get_root().find_child("Mano", true, false)

func _process(delta):
	if clicked_card and mouse_press_time != 0 and (Time.get_ticks_msec() - mouse_press_time) >= 200:
		selected_card = clicked_card
		selected_card.raise()
		selected_card.custom_minimum_size = selected_card.size
...
func finish_mouse_click():
	if clicked_card:
		var elapsed_time = Time.get_ticks_msec() - mouse_press_time
		if elapsed_time < 100:
...
func move_selected_card():
	if selected_card and not is_location_card:
		...
		selected_card.global_position = initial_card_position + (current_mouse_position - initial_mouse_position)
					...
					for slot in card_slots:
						if slot.name == "card_slot" or slot.name == "card_slot2" or slot.name == "card_slot3" or slot.name == "card_slot4":
							if not slot.get_children():
								...
								# Escala el nodo Card
								new_card.scale = Vector2(0.6, 0.6) # Ajusta el valor para el tamaño deseado
